#!/usr/bin/env python3
"""
时间-公平对比：Noise-aware Valley+Planck 的额外迭代 vs 基线

1. Plain Adam：随机多起点 Adam（约 28s）。
2. Original Valley：粗网格+多起点 Adam（约 28s）。
3. LBL：分层训练+全参数 Adam polish（约 28s）。
4. Noise-aware Valley+Planck（迭代 150）：Sobol+LCB+投影候选+Planck-SPSA×150。
   耗时目标：约 35s（与 Adam 系 28s 接近，稍高用于补偿批处理固有优势）。

核心问题：把 noise-aware 的速度优势转化为更多迭代，质量能提升多少？

依赖同目录文件：maxcut_noise_aware_valley_planck.py

运行：
    /usr/bin/python3 maxcut_time_fair_comparison.py --trials 3
快速验证：
    /usr/bin/python3 maxcut_time_fair_comparison.py --fast
"""
...
CONFIG: dict[str, Any] = {
    ...
    # 时间-公平：双种子 × 75 迭代 = 总 SPSA 步数 150，总耗时 ~36-50s，
    # 与 Adam 系 ~45-85s 在同一量级，同时双种子覆盖更多地形。
    "noise_aware_local_iterations": 75,
    "output_dir": "maxcut_time_fair_results",
}
...
def summarize_and_plot(
    ...
    figure.suptitle(
        ...
        f"(noise-aware: {CONFIG['noise_aware_local_iterations']} SPSA iters)"
    )
    figure.tight_layout()
...
def main():
    ...
    print("=" * 78)
    print("时间-公平对比：Noise-aware Valley+Planck(iter=150) vs 三个基线(iter~28s)")
    print("=" * 78)
    print(
    ...
    for trial in range(int(CONFIG["trials"])):
        ...
        # Noise-aware 双种子：跑两次独立的 Sobol 扫描+投影+Planck-Adam，
        # 取两者中 true_cut_fraction 最高者，用速度换覆盖→精度。
        navp_seeds = [trial_seed + 3, trial_seed + 7]
        navp_results = []
        for si, ns in enumerate(navp_seeds):
            navp_results.append(noise_aware_valley_search(
                p2_objective, float(CONFIG["noise_sigma"]), ns
            ))
        best_navp = max(navp_results, key=lambda r: r["true_cut_fraction"])
        # 合并指标：耗时与评估数求和，true 取最大。
        best_navp_merged = dict(best_navp)
        best_navp_merged["runtime"] = sum(r["runtime"] for r in navp_results)
        best_navp_merged["evals"] = sum(r["evals"] for r in navp_results)
        best_navp_merged["estimator_calls"] = sum(
            r["estimator_calls"] for r in navp_results
        )
        best_navp_merged["point_evaluations"] = sum(
            r["point_evaluations"] for r in navp_results
        )
        best_navp_merged["history"] = navp_results[0]["history"] + navp_results[1]["history"]
        ...
        results = {
            ...
            "Noise-aware Valley + Planck": best_navp_merged,
        }
